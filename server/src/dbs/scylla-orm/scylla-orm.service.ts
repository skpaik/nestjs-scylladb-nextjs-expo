...
export class ScyllaOrmService {
  ...
  insert<T extends Record<string, any>>(entity: T): { query: string; params: any[] } {
  ...
  update<T extends Record<string, any>>(entity: T, where: Partial<T>): { query: string; params: any[] } {
    const table = getSchemaTable(entity);
    ...
    const params = [...fields.map(f => entity[f]), ...Object.keys(where).map(k => (where as any)[k])];
    return { query, params };
  }
  ...
  mapQueryParams<T extends Record<string, any>>(query: string, data: T): any[] {
    ...
    if (placeholders !== keys.length) {
      throw new Error(`Expected ${placeholders} values, but got ${keys.length}`);
    }
